...
qc.df = data.frame(presence = apply(af.dm.clean, 1, FUN = function(x) { length(which(x != 0))}),
                   variance = rowVars(af.dm.clean),
                   vmr = apply(af.dm.clean, 1, FUN = function(x) { litteR::iod(x)}),
                   mean.vaf = apply(af.dm.clean, 1, FUN = function(x) { mean(x[which(x != 0)]) }))
...
Heatmap(af.dm.clean[germline.mutations,cells.sample], show_row_names = T, show_column_names = F, cluster_rows = T, cluster_columns = T, 
...
Heatmap(af.dm.clean[c(donor.variants, recipient.variants),], show_row_names = T, show_column_names = F, cluster_rows = T, cluster_columns = T, 
...
deconvolution.df = data.frame(donor = colMeans(af.dm.clean[donor.variants,]),
