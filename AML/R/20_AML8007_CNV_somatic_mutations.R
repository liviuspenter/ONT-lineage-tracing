...
cnv.data.2 = cnv.data.2 %>% select(bc, seg, p_cnv) %>% filter(seg %in% c('1a', '3a', '5b')) %>% tidyr::pivot_wider(names_from = 'seg', values_from = 'p_cnv')
...
mutations.df.condensed = merge(mutations.df.condensed, 
                               tidyr::pivot_wider(data=mutations.df[,c('bc', 'mutated', 'gene')], 
                                                  values_from = 'mutated', names_from = 'gene')[,c('bc','DNMT3A', 'TP53.1', 'TP53.2')], 
      by = 'bc', all.x = T)
...
ha = columnAnnotation(celltype = mutations.df.condensed[cells.4, 'predicted.celltype'],
