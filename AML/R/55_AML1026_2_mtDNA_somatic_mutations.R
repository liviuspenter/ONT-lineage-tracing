...
ha = columnAnnotation(celltype = AML1026.2$predicted.celltype[colnames(df)],
                      col = list(celltype = AML.combined.colors),
                      simple_anno_size = unit(5, 'pt'), border=T)
svglite::svglite('./AML/figures/mtDNA/heatmaps/20220805_AML1026_mtDNA_NRAS_SF3B1_SRSF2.svg', width = 4.5, height = 1.5)
Heatmap(df, cluster_rows = F, cluster_columns = F, show_row_names = T, show_column_names = F, 
