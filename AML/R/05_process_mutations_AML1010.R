...
NRAS.statistics = NRAS.vaf %>% group_by(predicted.celltype) %>% summarize(NRAS.mut = length(which(mutated == 'mutated')),
...
ggplot(NRAS.statistics[which(NRAS.statistics$predicted.celltype %in% c('HSC', 'LMPP', 'GMP', 'CD14 Mono', 'CD16 Mono', 'cDC2', 'Prog_DC')),], 
