...
cluster.colors <- c(
  "Progenitor" = "firebrick", "Erythropoiesis" = "grey", "B lymphopoiesis" = "darkgreen",
...
p <- DimPlot(AML3005.13,
  reduction = "umap",
  cells.highlight = list(
    "mutated" = DNMT3A.combined$bc[which(DNMT3A.combined$mutated == "mutated")],
...
DNMT3A.condensed <- DNMT3A.combined %>%
  filter(bc %in% colnames(AML3005.13)) %>%
  group_by(manual.cluster) %>%
  summarize(
    mutated.1 = length(which(mutated == "mutated" & sample == "AML3005.1")),
...
p <- DimPlot(AML3005.13,
  reduction = "umap",
  cells.highlight = list(
    "mutated" = U2AF1.combined$bc[which(U2AF1.combined$mutated == "mutated")],
...
U2AF1.condensed <- U2AF1.combined %>%
  filter(bc %in% colnames(AML3005.13)) %>%
  group_by(manual.cluster) %>%
  summarize(
    mutated.1 = length(which(mutated == "mutated" & sample == "AML3005.1")),
...
p <- DimPlot(AML3005.13,
  reduction = "umap",
  cells.highlight = list(
    "mutated" = TP53.combined$bc[which(TP53.combined$mutated == "mutated")],
