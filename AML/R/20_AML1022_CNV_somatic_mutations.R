...
cnv.data = cnv.data %>% select(cell, seg, p_cnv) %>% filter(seg %in% c('20b')) %>% tidyr::pivot_wider(names_from = 'seg', values_from = 'p_cnv')
...
mutations.df.condensed = merge(mutations.df.condensed, 
                               tidyr::pivot_wider(data=mutations.df[,c('bc', 'mutated', 'gene')], 
                                                  values_from = 'mutated', names_from = 'gene')[,c('bc','DNMT3A', 'RUNX1', 'SF3B1')], 
      by = 'bc', all.x = T)
