...
DimPlot(boo, reduction = 'ref.umap', cells.highlight = list('mutated' = mutation.df$Cell[which(mutation.df$mutated == 'mutated')],
...
View(mutation.df %>% filter(timepoint == 'D0') %>% group_by(predicted.celltype, identifier) %>% summarize(mut.cells = length(which(mutated == 'mutated')),
                                                                wt.cells = length(which(mutated == 'wildtype')),
                                                                mut.freq = length(which(mutated == 'mutated')) / length(predicted.celltype)))

View(mutation.df %>% filter(predicted.celltype == 'Prog_RBC') %>% group_by(identifier) %>% summarize(mut.cells = length(which(mutated == 'mutated')),
                                                                wt.cells = length(which(mutated == 'wildtype')),
                                                                mut.freq = length(which(mutated == 'mutated')) / length(predicted.celltype)))
