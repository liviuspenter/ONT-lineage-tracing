...
df <- as.data.frame(t(bind_cols(
  af.dm.1[c("10685G>A", "15615G>A"), intersect(colnames(AML.all)[which(AML.all$orig.ident %in% c("AML1026.1"))], colnames(af.dm.1))],
...
boo <- df %>%
  ...
  summarize(
    mt.10685 = length(which(`10685G>A` != 0)),
...
ggplot(
  data = AML.all.df[which(grepl("AML1026", AML.all.df$Var2) & AML.all.df$Var1 %in% c("HSC", "LMPP", "GMP")), ],
...
ggplot() +
  geom_bar(
    data = boo[which(boo$predicted.celltype %in% c("HSC")), ],
    aes(x = sample, y = 100 * mt.10685.freq, fill = predicted.celltype), stat = "identity", position = "identity"
  ) +
  geom_bar(
    data = boo[which(boo$predicted.celltype %in% c("LMPP")), ],
    aes(x = sample, y = 100 * mt.10685.freq, fill = predicted.celltype), stat = "identity", position = "identity"
  ) +
  geom_bar(
    data = boo[which(boo$predicted.celltype %in% c("GMP")), ],
    aes(x = sample, y = 100 * mt.10685.freq, fill = predicted.celltype), stat = "identity", position = "identity"
  ) +
