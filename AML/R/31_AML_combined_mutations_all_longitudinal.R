...
AML.longitudinal <- data.frame(
  bc = c(colnames(AML1002), colnames(AML1007), colnames(AML3003), colnames(AML3005), colnames(AML8007)),
  predicted.celltype = c(
    AML1002$predicted.celltype,
  ...
  UMAP = rbind(
    AML1002@reductions$ref.umap@cell.embeddings,
...
mutations.df.condensed <- mutations.df %>%
  group_by(bc) %>%
  summarize(
    mutation = ifelse("mutated" %in% mutated, "mutated", "wildtype"),
...
ggplot(
  boo[which(boo$predicted.celltype %in% c("HSC", "LMPP", "GMP", "Prog_Mk", "Prog_RBC")), ],
  ...
  scale_x_discrete(limits = c(
    "HSC.Screening", "HSC.CR", "LMPP.Screening", "LMPP.CR", "GMP.Screening", "GMP.CR",
