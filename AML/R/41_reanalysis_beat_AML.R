...
ggplot(
  data = combined.data[which(!combined.data$specificDxAtAcquisition %in% c("unknown", "Unknown")), ],
  aes(x = reorder(specificDxAtAcquisition, vG.HSC, FUN = median), y = vG.HSC)
) +
  geom_boxplot() +
...
ggplot(
  data = combined.data[which(!combined.data$specificDxAtAcquisition %in% c("unknown", "Unknown")), ],
  aes(x = reorder(specificDxAtAcquisition, vG.progenitor, FUN = median), y = vG.progenitor)
) +
  geom_boxplot() +
  geom_jitter(size = 0.5) +
...
ggplot(
  data = combined.data[which(!combined.data$specificDxAtAcquisition %in% c("unknown", "Unknown")), ],
  aes(x = reorder(specificDxAtAcquisition, vG.GMP, FUN = median), y = vG.GMP)
) +
  geom_boxplot() +
  geom_jitter(size = 0.5) +
...
ggplot(
  data = combined.data[which(!combined.data$specificDxAtAcquisition %in% c("unknown", "Unknown")), ],
  aes(x = reorder(specificDxAtAcquisition, vG.promono, FUN = median), y = vG.promono)
) +
  geom_boxplot() +
  geom_jitter(size = 0.5) +
...
ggplot(
  data = combined.data[which(!combined.data$specificDxAtAcquisition %in% c("unknown", "Unknown")), ],
  aes(x = reorder(specificDxAtAcquisition, vG.mono, FUN = median), y = vG.mono)
) +
  geom_boxplot() +
  geom_jitter(size = 0.5) +
...
ggplot(
  data = combined.data[which(!combined.data$specificDxAtAcquisition %in% c("unknown", "Unknown")), ],
  aes(x = reorder(specificDxAtAcquisition, vG.cDC, FUN = median), y = vG.cDC)
) +
  geom_boxplot() +
  geom_jitter(size = 0.5) +
...
ggplot(
  data = combined.data[which(!combined.data$specificDxAtAcquisition %in% c("unknown", "Unknown")), ],
  aes(x = reorder(specificDxAtAcquisition, Erythroid, FUN = median), y = Erythroid)
) +
  geom_boxplot(outlier.size = 0) +
  geom_jitter(size = 0.5, aes(color = Erythroid)) +
...
ggplot(
  data = combined.data[which(!combined.data$specificDxAtAcquisition %in% c("unknown", "Unknown")), ],
  aes(x = reorder(specificDxAtAcquisition, Megakaryocytic, FUN = median), y = Megakaryocytic)
) +
  geom_boxplot(outlier.size = 0) +
  geom_jitter(size = 0.5, aes(color = Megakaryocytic)) +
...
Heatmap(scale(mat),
  cluster_columns = F, cluster_rows = T, show_row_dend = F, row_names_side = "left", border = T,
...
ggplot(data = combined.data, aes(x = as.numeric(hemoglobin), y = as.numeric(HBB))) +
  geom_point(aes(color = priorMDS), size = 0.5) +
...
ggplot(data = combined.data[!is.na(combined.data$`%.Blasts.in.PB`), ], aes(x = as.numeric(`%.Blasts.in.PB`), y = as.numeric(HBB))) +
  geom_point(aes(color = priorMDS), size = 0.5) +
...
ggplot(data = combined.data[!is.na(combined.data$`%.Blasts.in.BM`), ], aes(x = as.numeric(`%.Blasts.in.BM`), y = as.numeric(HBB))) +
  geom_point(aes(color = priorMDS), size = 0.5) +
...
ggplot(data = combined.data[!is.na(combined.data$wbcCount), ], aes(x = as.numeric(`%.Blasts.in.BM`), y = as.numeric(HBB))) +
  geom_point(aes(color = priorMDS), size = 0.5) +
...
so <- AddModuleScore(so, features = list("vG.HSC" = c(
  "NPTX2", "H1F0", "EMP1", "CALCRL", "TPSD1", "TPT1", "CRHBP", "TSC22D1", "DST",
  "NRIP1", "ABCB1", "ZBTB20", "TPSB2", "KMT2A", "MEF2C", "ST3GAL1", "TMEM25", "C20orf203", "GNG11", "HOPX"
)), ctrl = 5, name = "vG.HSC")
...
so <- AddModuleScore(so, features = list("vG.GMP" = c(
  "PRTN3", "MPO", "CALR", "CLEC5A", "ELANE", "TRH", "CEBPE", "NUCB2", "CSF3R", "CD38", "IGFBP2",
  "PRRT4", "SNHG5", "FABP5", "CLEC11A", "SERPINB1", "AZU1", "HNRNPDL", "HSPB1", "C12orf57", "FGFR1"
)), ctrl = 5, name = "vG.GMP")
so <- AddModuleScore(so, features = list("vG.promono" = c(
  ...
  "FAM107B", "KBTBD11", "CSTA", "ANKRD28", "PIWIL4", "RNVU1-6"
)), ctrl = 5, name = "vG.promono")
so <- AddModuleScore(so, features = list("vG.mono" = c(
  "FCN1", "S100A12", "MAFB", "S100A9", "PLBD1", "SERPINA1", "BCL2A1", "THBS1", "PSAP", "S100A8", "FPR1", "C5AR1",
  "CD14", "NAMPT", "VNN2", "CTSS", "DUSP1", "CEBPB", "CR1", "NFKBIA", "SLC11A1", "LILRB3", "BCL6",
  "CYP1B1", "TNFAIP2", "AQP9", "TLR4", "APOBEC3A"
)), ctrl = 5, nbin = 5, name = "vG.mono")
so <- AddModuleScore(so, features = list("vG.cDC" = c(
  "HLA-DRB5", "CST3", "SAMHD1", "NAPSB", "FCER1A", "HLA-DRB1", "PKIB", "HLA-DRA", "HLA-DRB6", "CPVL",
  "HLA-DPB1", "HLA-DQA1", "HLA-DPA1", "CLEC4A", "TMSB10", "CAP1", "HLA-DQB1", "CRIP1", "CLEC10A", "GPX1",
  "ITGB7", "HLA-DQB2", "DBI", "FTH1P3", "ACTB", "HLA-DQA2", "S100B", "ALDH2"
)), ctrl = 5, name = "vG.cDC")
...
lgd_hemoglobin <- Legend(
  title = "hemoglobin", col_fun = col_fun2, at = c(0, 5, 10, 15, 20),
  labels = c("0", "5", "10", "15", "20")
)

ht <- Heatmap(beataml.RNA[marker.genes.RNA, beataml.meta$dbgap_rnaseq_sample],
  cluster_rows = F, show_row_dend = F, show_column_dend = F, show_row_names = T, show_column_names = F,
...
Heatmap(
  beataml.RNA[c(
    signature.van.Galen$HSC,
    signature.van.Galen$progenitor,
    signature.van.Galen$GMP,
    signature.van.Galen$promono,
    signature.van.Galen$mono,
    signature.van.Galen$cDC
  ), beataml.meta$dbgap_rnaseq_sample],
  cluster_rows = F, show_row_dend = F, show_column_dend = F,
...
sample.identity <- combined.data[, c(
  "vG.HSC.rel", "vG.progenitor.rel", "vG.GMP.rel", "vG.promono.rel",
  "vG.mono.rel", "vG.cDC.rel", "Erythroid.rel", "Megakaryocytic.rel"
)] %>% mutate(identity = names(.)[max.col(.)])


boo <- beataml.meta %>%
  group_by(dbgap_rnaseq_sample) %>%
  summarize(dbgap_rnaseq_sample, overallSurvival = unique(overallSurvival), vitalStatus)
boo$status <- NA
