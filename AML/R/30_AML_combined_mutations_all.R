...
p <- DimPlot(AML.subset,
  sizes.highlight = 0.5,
  reduction = "ref.umap",
  cells.highlight = list(
    "mutated" = mutations.df.condensed$bc[which(mutations.df.condensed$mutation == "mutated")],
...
boo <- mutations.df.condensed %>%
  group_by(predicted.celltype, patient) %>%
...
ha2 <- rowAnnotation(
  celltype = c(myeloid.clusters, "Prog_Mk", "Prog_RBC", TNK.clusters),
...
Heatmap(boo,
  na_col = "white", cluster_rows = F, cluster_columns = T, show_column_dend = F, col = col_fun, row_names_side = "left", border = T,
...
boo <- mutations.df.condensed %>%
  group_by(predicted.celltype, patient) %>%
  summarize(
    mut.cells = length(which(mutation == "mutated")),
