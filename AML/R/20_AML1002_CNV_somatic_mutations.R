...
mutations.df.condensed <- merge(mutations.df.condensed,
  ...
  )[, c("bc", "IDH2", "TP53.1", "TP53.2", "TET2", "U2AF1")],
  by = "bc", all.x = T
)
...
ggplot(mutations.df.condensed[which(mutations.df.condensed$predicted.celltype %in% c("NK cells")), ], aes(x = `7b`, y = NCAM1)) +
  geom_point(aes(color = TP53.mutated))
ggplot(mutations.df.condensed[which(mutations.df.condensed$predicted.celltype %in% c("CD4")), ], aes(x = `7b`, y = CD4)) +
  geom_point(aes(color = TP53.mutated))
...
ha <- columnAnnotation(
  celltype = mutations.df.condensed[cells.3, "predicted.celltype"],
...
ha <- columnAnnotation(
  celltype = mutations.df.condensed[c(cells.1, cells.3), "predicted.celltype"],
...
###
boo <-
  mutations.df.condensed %>%
  group_by(predicted.celltype) %>%
  summarize(
    IDH2.freq = length(which(IDH2 == "mutated")) / length(which(!is.na(IDH2))),
...
boo[is.na(boo)] <- 0
ggplot(
  boo[which(boo$predicted.celltype %in% c("HSC", "LMPP", "GMP", "Prog_Mk", "Prog_RBC", "CD4", "CD8", "NK cells")), ],
  ...
  scale_y_discrete(
    limits = rev(c("IDH2.freq", "TP53.1.freq", "TP53.2.freq", "TET2.freq", "U2AF1.freq")),
