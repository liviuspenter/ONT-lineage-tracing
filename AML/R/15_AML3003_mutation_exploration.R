...
ComplexHeatmap::Heatmap(
  t(mutations.3003[
    which(grepl("AML3003.1", rownames(mutations.3003))),
...
ComplexHeatmap::Heatmap(
  t(mutations.3003[
    which(grepl("AML3003.4", rownames(mutations.3003))),
    c("ASXL1", "STAG2", "TET2.2", "TET2.3", "TET2.4")
  ]),
  show_column_names = F, col = c("mutated" = "firebrick", "wildtype" = "white"), na_col = "white",
  column_split = celltype, column_title_side = "bottom", column_title_rot = 90, column_title_gp = gpar(fontsize = 8),
...
statistics.3003 <- mutations.3003 %>%
  ...
  summarize(
    ASXL1.mut = length(which(ASXL1 == "mutated")),
...
p <- DoHeatmap(boo,
  group.by = "mutated", features = highlight.genes, disp.max = 2,
