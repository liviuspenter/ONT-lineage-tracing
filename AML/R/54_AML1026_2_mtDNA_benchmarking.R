...
AML1026.combined.frequencies = 
  AML1026.combined.frequencies[,intersect(colnames(AML1026.combined.frequencies), 
...
ggplot(benchmarking.df, aes(x=mtscATAC, y=scRNAseq)) + geom_point() + scale_x_log10() + scale_y_log10()
...
af.dm.clean.scatac = af.dm.clean.scatac[,intersect(colnames(af.dm.clean.scatac), 
                                                   asap.cells.by.sample$barcode[which(asap.cells.by.sample$sample == 'AML1026_2')])]
...
p=ggplot() + 
  geom_abline(slope = 1) + 
  ggrastr::rasterize(geom_point(data=benchmarking.df[which(!rownames(benchmarking.df) %in% rownames(AML1026.combined.frequencies)),], 
                                aes(x=mtscATAC, y=scRNAseq), size=0.5, color='grey'), dpi=600) + 
  ggrastr::rasterize(geom_point(data=benchmarking.df[which(rownames(benchmarking.df) %in% rownames(AML1026.combined.frequencies)),], 
                                aes(x=mtscATAC, y=scRNAseq), size=0.5, color='firebrick'), dpi=600) + 
