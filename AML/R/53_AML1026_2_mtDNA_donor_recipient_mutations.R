...
Heatmap(af.dm.clean[mtDNA.variants, TNK.cells],
  show_row_dend = F, show_column_dend = F, show_row_names = T, row_names_gp = gpar(fontsize = 6),
...
ha <- columnAnnotation(
  ...
  col = list(celltype = c(
    "HSC" = as.character(AML.combined.colors["HSC"]),
...
Heatmap(
  af.dm.clean[
    c("10685G>A", "3106C>A", "3106C>G", "3106C>T", "15615G>A", "9254A>G", donor.variants, recipient.variants),
    c(HSC.cells, GMP.cells, LMPP.cells, CD4.recipient, NK.recipient, CD4.donor, NK.donor)
  ],
  top_annotation = ha,
  column_split = factor(
    c(
