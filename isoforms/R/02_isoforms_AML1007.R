...
ggplot(PTPRC, aes(x = CD45RA, y = detected.rel)) +
  geom_point(size = 0.5) +
...
PTPRC.statistics.1007 <- PTPRC.1007 %>%
  group_by(sample, predicted.celltype) %>%
  summarize(
    cells.detected = length(which(detected > 0)),
...
ggplot(
  PTPRC.statistics.1007[which(PTPRC.statistics.1007$predicted.celltype %in% c("CD4 Naive", "CD4 Memory", "CD8 Naive", "CD8 Memory_2")), ],
...
p <- DimPlot(AML1007,
  cells = colnames(AML1007)[which(AML1007$orig.ident == "AML1007.1")],
  reduction = "ref.umap", cells.highlight = list(
    "detected" = PTPRC.1007$bc[which(PTPRC.1007$detected > 0)],
...
q <- DimPlot(AML1007,
  cells = colnames(AML1007)[which(AML1007$orig.ident == "AML1007.3")],
  reduction = "ref.umap", cells.highlight = list(
    "detected" = PTPRC.1007$bc[which(PTPRC.1007$detected > 0)],
    "not.detected" = PTPRC.1007$bc[which(PTPRC.1007$not.detected > 0)]
  ),
...
r <- DimPlot(AML1007,
  cells = colnames(AML1007)[which(AML1007$orig.ident == "AML1007.5")],
  reduction = "ref.umap", cells.highlight = list(
    "detected" = PTPRC.1007$bc[which(PTPRC.1007$detected > 0)],
    "not.detected" = PTPRC.1007$bc[which(PTPRC.1007$not.detected > 0)]
  ),
