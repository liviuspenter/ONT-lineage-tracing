...
# read isoform data
PTPRC.WT <- extract_isoforms("./data/isoforms/AML3005.1/AML3005_1_WT_exons.gz", GENE = "PTPRC", EXON = "exon4", filter = 2)
PTPRC.WT$sample <- "AML3005.1"
...
PTPRC.3005 <- PTPRC
PTPRC.statistics.3005 <- PTPRC.3005 %>%
...
ggplot(
  PTPRC.statistics.3005[which(PTPRC.statistics.3005$predicted.celltype %in% c("CD4 Naive", "CD4 Memory", "CD8 Naive", "CD8 Memory_2")), ],
...
PTPRC <- PTPRC %>%
  group_by(sample, gene.exon, bc) %>%
  summarize(n = n())
PTPRC <- PTPRC %>%
  arrange(desc(n)) %>%
  group_by(sample, gene.exon) %>%
