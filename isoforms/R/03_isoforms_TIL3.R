...
ggplot() +
  geom_point(
    data = PTPRC.TIL3[order(PTPRC.TIL3$detected / (PTPRC.TIL3$detected + PTPRC.TIL3$not.detected)), ],
...
ggplot() +
  geom_point(
    data = PTPRC.PBMC8[order(PTPRC.PBMC8$detected / (PTPRC.PBMC8$detected + PTPRC.PBMC8$not.detected)), ],
    aes(x = UMAP1, y = UMAP2, color = detected / (detected + not.detected)), size = 0.5
  ) +
  scale_color_gradientn(colors = c("grey90", BuenColors::jdb_palette(name = "solar_rojos"))) +
  theme_classic() +
