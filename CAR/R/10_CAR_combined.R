...
T.cell.subsets = c('CD4 TCM', 'CD4 TEM', 'CD4 Proliferating', 'Treg', 
                   'CD8 Naive', 'CD8 TCM', 'CD8 TEM', 'CD8 Proliferating')

T.cell.subset.colors = c('CD4 TCM' = RColorBrewer::brewer.pal(name = 'Blues', n=6)[3],
...
p=DimPlot(CAR.data, cols = 'grey90', cells.highlight = list('CD4' = colnames(CAR.data)[which(CAR.data$predicted.celltype.l1 == 'CD4 T')],
...
p=FeaturePlot(CAR.data, cells = CAR.df$bc, 
...
boo = CAR.df %>% filter(CARTmod != 0) %>% group_by(sample, timepoint, predicted.celltype.l1) %>% 
...
p=ggplot(data=CAR.df[which(!is.na(CAR.df$freq) & CAR.df$predicted.celltype.l2 %in% names(T.cell.subset.colors)),], 
...
boo = CAR.df %>% group_by(exhausted, high.expression) %>% tally() %>% tidyr::pivot_wider(names_from = c('exhausted', 'high.expression'), values_from = 'n')
