...
ggplot() +
  ggrastr::rasterize(geom_point(
    data = CAR.data.combined[which(CAR.data.combined$CARTmod != 0 | CAR.data.combined$CART_count != 0), ],
    aes(x = CARTmod.plot, y = CART_count.plot, color = technology.detected), size = 0.5
  ), dpi = 600) +
...
ggplot(
  data.frame(
    condition = c("Both", "ONT", "Illumina"),
    n = c(
      length(which(CAR.data.combined$CARTmod > 4 & CAR.data.combined$CART_count > 4)),
