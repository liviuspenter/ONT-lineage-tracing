...
CAR.data.combined <- CAR.data.combined %>%
  arrange(-CARTmod.CD28) %>%
  mutate(ONT.rank.CD28 = row_number())
CAR.data.combined <- CAR.data.combined %>%
  arrange(-CARTmod.CD247) %>%
  mutate(ONT.rank.CD247 = row_number())
CAR.data.combined <- CAR.data.combined %>%
  arrange(-CART_count) %>%
  mutate(Illumina.rank = row_number())
...
ggplot() +
  ggrastr::rasterize(geom_point(
    data = CAR.data.combined[which(CAR.data.combined$CARTmod.CD247 != 0 | CAR.data.combined$CART_count != 0), ],
    aes(x = CARTmod.CD247.plot, y = CART_count.plot, color = technology.detected), size = 0.5
  ), dpi = 600) +
...
lt <- list(
  CD28 = CAR.data.combined$bc[which(CAR.data.combined$CARTmod.CD28 != 0)],
...
UpSet(m,
  ...
  comb_col = c(
    technology.colors["ONT"], technology.colors["ONT"], technology.colors["ONT"],
