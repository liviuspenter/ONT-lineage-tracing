...
# combine data
Illumina.reads <- Illumina.data %>%
  group_by(barcode, chain) %>%
  summarize(reads = sum(reads))
ONT.reads <- rbind(
  ONT.data %>% group_by(barcode) %>% summarize(chain = "TRB", reads = sum(as.numeric(strsplit(TRB_counts, split = "_")[[1]]))),
...
combined.reads <- rbind(
  merge(Illumina.reads[which(Illumina.reads$chain == "TRA"), ],
...
p <- ggplot(
  combined.reads[sample(nrow(combined.reads), nrow(combined.reads), replace = F), ],
...
combined.reads <- combined.reads %>%
  group_by(ONT.chain) %>%
  arrange(-ONT.reads) %>%
  mutate(rank.ONT = row_number())
combined.reads <- combined.reads %>%
  group_by(Illumina.chain) %>%
  arrange(-Illumina.reads) %>%
  mutate(rank.Illumina = row_number())

# compare library structures
ggplot() +
  geom_line(
    data = combined.reads[which(combined.reads$ONT.chain == "TRA" & combined.reads$ONT.reads != 0), ],
    aes(x = rank.ONT, y = ONT.reads), color = "blue"
  ) +
  geom_line(
    data = combined.reads[which(combined.reads$Illumina.chain == "TRA" & combined.reads$Illumina.reads != 0), ],
...
ggplot() +
  geom_line(
    data = combined.reads[which(combined.reads$ONT.chain == "TRB" & combined.reads$ONT.reads != 0), ],
    aes(x = rank.ONT, y = ONT.reads), color = "blue"
  ) +
  geom_line(
    data = combined.reads[which(combined.reads$Illumina.chain == "TRB" & combined.reads$Illumina.reads != 0), ],
...
ONT.data$CDR3b <- lapply(ONT.data$TRBs, FUN = function(x) {
  stringr::str_split(x, pattern = "_")[[1]]
})
ONT.data$CDR3a <- lapply(ONT.data$TRAs, FUN = function(x) {
  stringr::str_split(x, pattern = "_")[[1]]
})
...
Illumina.cdr3a <- Illumina.data %>%
  group_by(barcode) %>%
  filter(chain == "TRA" & cdr3 != "None") %>%
  summarize(cdr3 = paste(sort(cdr3), collapse = "_"))
Illumina.cdr3a$number <- unlist(lapply(Illumina.cdr3a$cdr3, FUN = function(x) {
  length(stringr::str_split(x, pattern = "_")[[1]])
}))
ONT.cdr3a <- ONT.data %>%
  group_by(barcode) %>%
  filter(TRAs != "None") %>%
  summarize(cdr3 = paste(sort(stringr::str_split(TRAs, pattern = "_")[[1]]), collapse = "_"))
ONT.cdr3a$number <- unlist(lapply(ONT.cdr3a$cdr3, FUN = function(x) {
  length(stringr::str_split(x, pattern = "_")[[1]])
}))
...
Illumina.cdr3b <- Illumina.data %>%
  group_by(barcode) %>%
  filter(chain == "TRB" & cdr3 != "None") %>%
  summarize(cdr3 = paste(sort(cdr3), collapse = "_"))
Illumina.cdr3b$number <- unlist(lapply(Illumina.cdr3b$cdr3, FUN = function(x) {
  length(stringr::str_split(x, pattern = "_")[[1]])
}))
ONT.cdr3b <- ONT.data %>%
  group_by(barcode) %>%
  filter(TRBs != "None") %>%
  summarize(cdr3 = paste(sort(stringr::str_split(TRBs, pattern = "_")[[1]]), collapse = "_"))
ONT.cdr3b$number <- unlist(lapply(ONT.cdr3b$cdr3, FUN = function(x) {
  length(stringr::str_split(x, pattern = "_")[[1]])
}))
...
Illumina.cdr3b$ONT.count <- sapply(Illumina.cdr3b$Var1, FUN = function(x) {
  length(which(grepl(x, ONT.data$TRBs)))
})
...
Illumina.cdr3a$ONT.count <- sapply(Illumina.cdr3a$Var1, FUN = function(x) {
  length(which(grepl(x, ONT.data$TRAs)))
})
