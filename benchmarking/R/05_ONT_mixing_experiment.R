...
mixing.data.1 = as.data.frame(nanoranger.R::extract_mutation(BC.data.file = './data/benchmarking/K562_Kasumi1_1/K562_Kasumi1_pileup_TP53.csv.gz', 
                                 ALT = 'T', REF = 'C'))
mixing.data.1$bc = paste0('K562.1_',mixing.data.1$bc,'-1')
rownames(mixing.data.1) = mixing.data.1$bc

mixing.data.10 = as.data.frame(nanoranger.R::extract_mutation(BC.data.file = './data/benchmarking/K562_Kasumi1_10/K562_Kasumi1_pileup_TP53.csv.gz', 
                                 ALT = 'T', REF = 'C'))
mixing.data.10$bc = paste0('K562.10_',mixing.data.10$bc,'-1')
rownames(mixing.data.10) = mixing.data.10$bc

mixing.data.50 = as.data.frame(nanoranger.R::extract_mutation(BC.data.file = './data/benchmarking/K562_Kasumi1_50/K562_Kasumi1_pileup_TP53.csv.gz', 
                                 ALT = 'T', REF = 'C'))
mixing.data.50$bc = paste0('K562.50_',mixing.data.50$bc,'-1')
rownames(mixing.data.50) = mixing.data.50$bc

mixing.data.99 = as.data.frame(nanoranger.R::extract_mutation(BC.data.file = './data/benchmarking/K562_Kasumi1_99/K562_Kasumi1_pileup_TP53.csv.gz', 
                                                ALT = 'T', REF = 'C'))
...
mutation.statistics = bind_rows(clusters.1 %>% filter(barcode %in% colnames(mixing.so)) %>%
                                  group_by(cell.line) %>% summarize(mut = length(which(mutated == 'mutated')),
...
TP53.insert.counts.1 = TP53.insert.counts.1 %>% filter(!is.na(count)) %>% 
...
TP53.insert.counts.10 = TP53.insert.counts.10 %>% filter(!is.na(count)) %>% 
  filter(cell.line %in% c('K562', 'Kasumi1')) %>%
  group_by(cell.line) %>%
  arrange(-count, .by_group = T) %>%
  mutate(rank = row_number())
...
TP53.insert.counts.50 = TP53.insert.counts.50 %>% filter(!is.na(count)) %>% 
  filter(cell.line %in% c('K562', 'Kasumi1')) %>%
  group_by(cell.line) %>%
  arrange(-count, .by_group = T) %>%
  mutate(rank = row_number())
...
TP53.insert.counts.99 = TP53.insert.counts.99 %>% filter(!is.na(count)) %>% 
  filter(cell.line %in% c('K562', 'Kasumi1')) %>%
  group_by(cell.line) %>%
  arrange(-count, .by_group = T) %>%
  mutate(rank = row_number())
...
mutation.statistics.insert = bind_rows(TP53.insert.counts.1 %>% group_by(cell.line) %>% summarize(mut = length(which(mutated == 'mutated')),
                                                                                                  wt = length(which(mutated == 'wildtype')),
                                                                                                  cells = length(which(mixing.so$orig.ident == 'K562.1' &
                                                                                                                         mixing.so$cell.line == cell.line)),
                                                                                                  sample = 'K562.1'),
                                       TP53.insert.counts.10 %>% group_by(cell.line) %>% summarize(mut = length(which(mutated == 'mutated')),
                                                                                                  wt = length(which(mutated == 'wildtype')),
                                                                                                  cells = length(which(mixing.so$orig.ident == 'K562.10' &
                                                                                                                         mixing.so$cell.line == cell.line)),
                                                                                                  sample = 'K562.10'),
                                       TP53.insert.counts.50 %>% group_by(cell.line) %>% summarize(mut = length(which(mutated == 'mutated')),
                                                                                                   wt = length(which(mutated == 'wildtype')),
                                                                                                   cells = length(which(mixing.so$orig.ident == 'K562.50' &
                                                                                                                          mixing.so$cell.line == cell.line)),
                                                                                                   sample = 'K562.50'),
                                       TP53.insert.counts.99 %>% group_by(cell.line) %>% summarize(mut = length(which(mutated == 'mutated')),
                                                                                                   wt = length(which(mutated == 'wildtype')),
...
for (reads in seq(10000, 500000, 10000)) {
  mixing.data.downsampled = as.data.frame(extract_mutation(BC.data.file = './data/Mehdi/K562_Kasumi1_50/K562_Kasumi1_pileup_TP53.csv.gz', 
  ...
  downsample.df = rbind(downsample.df, data.frame(reads = reads, 
...
BCRABL1.statistics = as.data.frame(BCRABL1.data %>% 
  filter(count > 100) %>% 
  group_by(cell.line) %>% 
  summarize(genotyped = length(cell.line),
...
p=ggplot() + 
  ggrastr::rasterize(ggpointdensity::geom_pointdensity(data=BC.data.condensed[which(BC.data.condensed$gene == 'BCR'),], 
...
q=ggplot() + 
  ggrastr::rasterize(ggpointdensity::geom_pointdensity(data=BC.data.condensed[which(BC.data.condensed$gene == 'RUNX1T1'),], 
                                                       aes(x=-qlen.median, y=count), size=0.5), dpi=600) +
  viridis::scale_color_viridis() +
