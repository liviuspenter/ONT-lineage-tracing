...
samples <- c(
  "Pool116_3" = "./data/benchmarking/WT_bulk_coverage/Pool116_3_pc_canon_coverage.txt.gz",
...
for (s in seq(5, 7)) {
  ...
  coverage.df <- coverage.df %>%
    group_by(transcript) %>%
    mutate(max.coverage = max(coverage))
  coverage.df <- coverage.df %>%
    group_by(transcript) %>%
    mutate(int.coverage = sum(coverage) / length(transcript))
  ...
  coverage.position <- coverage.df %>%
    group_by(position, length.bin) %>%
    summarize(
      mean.coverage = mean(rel.coverage),
      abs.coverage = mean(coverage),
      transcripts = length(unique(transcript))
    )
  coverage.position <- coverage.position %>%
    group_by(length.bin) %>%
    mutate(max.transcripts = max(transcripts))
  ...
  coverage.position <- coverage.df %>%
    group_by(position_3p, length.bin) %>%
    summarize(
      mean.coverage = mean(rel.coverage),
      abs.coverage = mean(coverage),
      transcripts = length(unique(transcript))
...
for (s in seq(1, 6)) {
  ...
  coverage.df.condensed <- coverage.df %>%
    group_by(transcript) %>%
    summarize(
