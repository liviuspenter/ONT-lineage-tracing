...
for (l in libraries) {
  ...
  stats.df <- rbind(stats.df, data.frame(
    library = l,
  ...
  read.stats <- function(df, condition = NA) {
    ...
    df.reads <- df.reads[-1, ]
    df.reads[1, ] <- sapply(as.numeric(df.reads[1, ]), FUN = function(x) {
      x / sum(as.numeric(df.reads[1, ]))
    })
...
Heatmap(reads.df[, seq(1, 30)],
  cluster_rows = F, cluster_columns = F, col = col_fun, border = T,
