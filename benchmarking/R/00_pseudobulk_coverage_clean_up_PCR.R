...
for (gene in c("RUNX1", "TP53")) {
  ...
  for (exon in seq_along(exones)) {
    exon.df <- rbind(exon.df, data.frame(
      start = min(coverage.sub$x[which(coverage.sub$exon == exon)]),
  ...
  primer.df <- data.frame(
    start = start(primers[which(grepl(gene, primers$name))]),
    end = end(primers[which(grepl(gene, primers$name))])
  )

  mutations.df <- data.frame(
    start = start(mutations[which(grepl(gene, mutations$name))]),
    end = end(mutations[which(grepl(gene, mutations$name))])
  )

  coverage.sub <- coverage.sub %>%
    group_by(sample) %>%
    mutate(reads.rel = reads / max(reads))

  p <- ggplot() +
    geom_line(data = coverage.sub, aes(x = x, y = 100 * reads.rel, color = sample)) +
    # primers
    geom_segment(
      data = primer.df, x = coverage.sub$x[which(coverage.sub$abs.position %in% primer.df$start & coverage.sub$sample == "DLI")],
    ...
    # mutations
    geom_segment(
      data = mutations.df, x = coverage.sub$x[which(coverage.sub$abs.position %in% mutations.df$start & coverage.sub$sample == "DLI")],
