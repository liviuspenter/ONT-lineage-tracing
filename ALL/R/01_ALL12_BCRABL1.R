...
ALL.cells <- c(
  rownames(ALL1.genotype)[which(ALL1.genotype$mutated == "mutated")],
...
ggplot(df, aes(x = UMAP_1, y = UMAP_2)) +
  ...
  scale_color_manual(values = c(
    "Ery" = "grey", "Progenitor" = "yellow", "ALL" = "darkorange", "B cell" = "firebrick", "CD4" = "lightblue", "CD8" = "darkblue",
...
DimPlot(ALL,
  reduction = "ref.umap", cells.highlight = list(
    "ALL.1" = rownames(ALL1.genotype)[which(ALL1.genotype$mutated == "mutated")],
...
p <- FeaturePlot(ALL, "BCR", cols = c("grey", BuenColors::jdb_palette(name = "brewer_red"))) + NoLegend() + NoAxes() +
  theme(plot.title = element_blank())
ggsave("./ALL/figures/UMAP/20221006_BCR.png", width = 4, height = 4, dpi = 600, plot = p)
