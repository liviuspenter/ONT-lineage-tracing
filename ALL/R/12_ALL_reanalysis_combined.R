...
ha2 <- rowAnnotation(
  celltype = c(myeloid.clusters, "Prog_Mk", "Prog_RBC", TNK.clusters, "Prog_B 1", "Prog_B 2", "Naive B", "Memory B", "Plasmablast"),
...
Heatmap(t(boo),
  cluster_rows = F, cluster_columns = F, col = col_fun, na_col = "white", border = T, row_names_side = "left",
  ...
  row_split = factor(c(
    rep("myeloid", length(myeloid.clusters)),
    rep("MK_RBC", 2),
    rep("TNK", length(TNK.clusters)),
    rep("B", 5)
  ), c("B", "myeloid", "MK_RBC", "TNK"))
)
dev.off()
