...
BCRABL1.coverage = BCRABL1.coverage %>% group_by(sample, gene) %>% mutate(coverage = coverage / max(coverage))

cov = ggplot() + geom_rect(data=annotation.df, aes(xmin=start, xmax=end, ymin=0, ymax=100, fill=name)) +
